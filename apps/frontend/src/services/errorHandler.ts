...
export class ErrorHandler {
  ...
  static handleApiError(error: unknown, context?: string): string {
    ...
    if (error instanceof Error) {
      ...
      log.error('api', `API Error${context ? ` in ${context}` : ''}`, {
      ...
      errorMessage = error;
      log.error('api', `API Error${context ? ` in ${context}` : ''}`, { message: error });
    } else {
      log.error('api', `Unknown API Error${context ? ` in ${context}` : ''}`, { error });
    }
  ...
  static handleValidationError(error: unknown, context?: string): string {
    ...
    if (error instanceof Error) {
      errorMessage = error.message;
      log.error('validation', `Validation Error${context ? ` in ${context}` : ''}`, {
        message: error.message,
        stack: error.stack
      });
    } else if (typeof error === 'string') {
      errorMessage = error;
      log.error('validation', `Validation Error${context ? ` in ${context}` : ''}`, { message: error });
    } else {
      log.error('validation', `Unknown Validation Error${context ? ` in ${context}` : ''}`, { error });
    }
  ...
  static handleAuthError(error: unknown, context?: string): string {
    ...
    if (error instanceof Error) {
      errorMessage = error.message;
      log.error('auth', `Auth Error${context ? ` in ${context}` : ''}`, {
        message: error.message,
        stack: error.stack
      });
    } else if (typeof error === 'string') {
      errorMessage = error;
      log.error('auth', `Auth Error${context ? ` in ${context}` : ''}`, { message: error });
    } else {
      log.error('auth', `Unknown Auth Error${context ? ` in ${context}` : ''}`, { error });
    }
  ...
  static handleDatabaseError(error: unknown, context?: string): string {
    if (error instanceof Error) {
      log.error('database', `Database Error${context ? ` in ${context}` : ''}`, {
        message: error.message,
        stack: error.stack
      });
    } else if (typeof error === 'string') {
      log.error('database', `Database Error${context ? ` in ${context}` : ''}`, { message: error });
    } else {
      log.error('database', `Unknown Database Error${context ? ` in ${context}` : ''}`, { error });
    }
